...
CDlgImageViewer::CDlgImageViewer(UINT nIDTemplate, CWnd* pParent)
...
CDlgImageViewer::CDlgImageViewer(CImage* pImage, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
	m_pScrollView = NULL;
	m_pImage = pImage;
...
CDlgImageViewer::CDlgImageViewer(BYTE* ptr, int width, int height, int rowlen, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
...
CDlgImageViewer::CDlgImageViewer(char* ptr, int width, int height, int rowlen, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
...
CDlgImageViewer::CDlgImageViewer(int* ptr, int width, int height, int rowlen, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
...
CDlgImageViewer::CDlgImageViewer(float* ptr, int width, int height, int rowlen, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
...
CDlgImageViewer::CDlgImageViewer(double* ptr, int width, int height, int rowlen, UINT nIDTemplate, CWnd* pParent)
	: CDialogEx(nIDTemplate, pParent)
{
...
BEGIN_MESSAGE_MAP(CDlgImageViewer, CDialogEx)
	...
	ON_COMMAND(ID_QUIT, &CDlgImageViewer::OnClose)
	ON_WM_TIMER()
	ON_WM_MOUSEWHEEL()
...
#define ITEMS 15  // 工具栏的图标数目
	...
	// 设置对话框图标
	SetIcon(AfxGetApp()->LoadIcon(IDI_DLG_ICON), TRUE);
...
void CDlgImageViewer::OnSize(UINT nType, int cx, int cy)
{
	...
	if(m_pScrollView->GetSafeHwnd())
		m_pScrollView->MoveWindow(10, 70, rect.right - 20 , rect.bottom - 70 - BOTTTOM_BAR_SIZE);
}
...
{
	MessageBox(_T("此DR图像转换程序\"DRConverter.exe\"\n由首都师范大学检测成像工程中心发布。"), _T("关于"), MB_OK | MB_ICONINFORMATION);
}


void CDlgImageViewer::OnClose()
{
}
...
BOOL CDlgImageViewer::OnToolTipText(UINT, NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case ID_SAVE_IMAGE:
		...
		break;
	case ID_ABOUT:
		pTTT->lpszText = _T("关于这个程序");
		break;
	case ID_QUIT:
		pTTT->lpszText = _T("关闭当前文档");
		break;
	}
	return TRUE;
}